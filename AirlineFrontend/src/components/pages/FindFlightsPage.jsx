...
const FindFlightsPage = () => {
    ...
    useEffect( () => {
        const fetchAirports = async () => {
            try {
                ...
                showError("Failed to load airports" + error.message);
            }
        };
    ...
    useEffect(() => {
        ...
        setSearchParams(initialParam);

        if (initialParam.departureIataCode || initialParam.arrivalIataCode)
            fetchFlights(initialParam)
        else
            setLoading(false);
    }, [location]);

    const fetchFlights = async (initialParam) => {
        try {
            setLoading(true);
            const response = await ApiService.searchFlights(
                initialParam.departureIataCode,
                initialParam.arrivalIataCode,
                initialParam.departureDate);
                            ...
                            {flights.map(flight => (
                                <div key={flight.id} className="flight-card">
                                    <div className="flight-header">
                                        ...
                                        <div className={`flight-status ${flight.flightStatus.toLowerCase()}`}>
                                            {flight.flightStatus}
                                        </div>
                                    </div>
